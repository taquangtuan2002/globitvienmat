...
public class RestVisitController {
	...
	public VisitDto startVisit(@PathVariable("id-patient") UUID id) {
		Visit result = visitService.startVisit(id);
	...
	public VisitDto saveIehVisit(@RequestBody IehVisitDto iehVisitDto) {
		Visit result = visitService.saveIehVisit(iehVisitDto);
		return new VisitDto(result);
	}

	@Secured({EmrConstants.ROLE_ADMIN})
	@RequestMapping(value = "/get-ieh/{id}", method = RequestMethod.GET)
	public IehVisitDto getIehDto(@PathVariable("id") UUID visitId) {
		IehVisitDto result = visitService.getIehVisit(visitId);
		return result;
	}
