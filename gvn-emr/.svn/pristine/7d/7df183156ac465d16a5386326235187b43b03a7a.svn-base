...
public class RestObservationController {
    ...
    public ResponseEntity<ObservationDto> saveObservation(@RequestBody Observation dto,@PathVariable UUID id) {
        ObservationDto result = new ObservationDto( observationService.saveOrUpdate(dto,id));
        return new ResponseEntity<>(result, HttpStatus.OK);
    }
    @Secured({EmrConstants.ROLE_ADMIN})
    @RequestMapping(value = "/save", method = RequestMethod.POST)
    public ResponseEntity<ObservationDto> saveObservation(@RequestBody Observation dto) {
        ObservationDto result = new ObservationDto( observationService.saveOrUpdate(dto,null));
        return new ResponseEntity<>(result, HttpStatus.OK);
    }
