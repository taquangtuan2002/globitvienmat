...
public class ServiceTypeServiceImpl extends GenericServiceImpl<ServiceType, UUID> implements ServiceTypeService{
	...
	@Override
	public List<ServiceType> getAllDto() {
		return serviceTypeRepos.findAll();
	}

	@Override
	public ServiceType getDtoById(UUID id) {
		if(id != null) {
			return serviceTypeRepos.getById(id);
		}
		return null;
	}

	@Override
	public ServiceType saveOrUpdate(UUID id, ServiceType serviceType) {
		if(serviceType != null) {
			...
			if(serviceType.getId() != null) {
				Optional<ServiceType> serviceTypeOptional = serviceTypeRepos.findById(serviceType.getId());
			...
			if(serviceType.getCode() != null && StringUtils.hasText(serviceType.getCode())) {
				entity.setCode(serviceType.getCode());
			}
