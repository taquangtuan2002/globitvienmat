...
public class ConceptTypeServiceImpl extends GenericServiceImpl<ConceptType, UUID> implements ConceptTypeService{
	...
	public Boolean deleteById(UUID id) {
		if (id != null) {
			ConceptType entity = null;
			Optional<ConceptType> conceptTOptional = repos.findById(id);
            if (conceptTOptional.isPresent()) {
            	entity = conceptTOptional.get();
            }
            if (entity != null) {
            repos.deleteById(id);
            return true;}
    ...
    @Override
    public ConceptType getConceptTypeByCode(String code) {
        if(code != null) {
            ConceptType conceptType = repos.getByCode(code);
            if(conceptType != null) {
                return conceptType;
            }
        }
