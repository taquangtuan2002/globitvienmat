...
@RestController
@RequestMapping("/api/health-care-service")
...
public class RestHealthCareServiceController {
	...
	@Secured({EmrConstants.ROLE_ADMIN})
	@RequestMapping(value = "/get-list-page", method = RequestMethod.GET)
	...
	public ResponseEntity<Boolean> deleteFakeVoided(@PathVariable("id") UUID id) {
		Boolean result = healCareServiceService.deleteFakeVoided(id);
		if(result == null) {
			return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);
		}
		return new ResponseEntity<>(result, HttpStatus.OK);
